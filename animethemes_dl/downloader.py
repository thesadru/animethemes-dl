...
import requests
import string
from animethemes_dl.animethemes import get_proper
from animethemes_dl.printer import fprint
from animethemes_dl.globals import Opts
    ...
    for status in statuses:
        for anime in data[status]:
            if (anime['mal_data']['score'] < Opts.Animelist.minscore or
                    to_mal_priority(
                    anime['mal_data']['priority_string']) < Opts.Animelist.minpriority
                    ):
                continue
...
def convert_ffmpeg(webm_filename, mp3_filename=None, save_folder=None):
    ...
    if save_folder is not None:
        mp3_filename = os.path.join(
            save_folder, os.path.basename(mp3_filename))
...
def download_theme(theme_data, webm_folder=None, mp3_folder=None, no_redownload=False):
    ...
    if no_redownload:
        ...
        needwebm = (webm_folder is not None and not exwebm) or (
            needmp3 and not exwebm)
    else:
        needwebm = True
...
def download_theme_audio_server(theme_data, mp3_folder=None, no_redownload=False, data_size=65536, timeout=30):
    ...
    def progress_bar():
        ...
        def current_time(): return time.time() - START_TIME
