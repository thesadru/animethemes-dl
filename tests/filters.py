...
from animethemes_dl.parsers.dldata import filter_download_data
...
for f in ('spoiler','nsfw', 'nc', 'subbed', 'lyrics', 'uncen'):
    none = len(filter_download_data(raw))
    for x in (True,False):
        OPTIONS['filter'][f] = x
        modified = len(filter_download_data(raw))
        logger.debug(f'Filter {f}:{int(x)} gives {modified}x{none}.')
        if modified==none:
            logger.info(f'Filter "{f}" does nothing when "{x}".')
